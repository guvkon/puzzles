module Utils exposing ( decimal
                      , parseStringIntoBlocks
                      , indexes
                      , element
                      , field
                      , rotateMatrix
...
stringToMatrix : String -> Dict.Dict (Int, Int) Int
stringToMatrix string =
    let
        stringToList : String -> List Int
        stringToList line =
            String.toList line
                |> List.filterMap (\char -> String.toInt (String.fromChar char))
        iFoldl =
    ...
    string
        |> String.trim
        |> String.lines
        |> List.map stringToList
        |> iFoldl (\x row matrix -> iFoldl (\y value rowMatrix -> Dict.insert (x, y) value rowMatrix) matrix row) Dict.empty
