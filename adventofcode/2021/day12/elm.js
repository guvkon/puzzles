...
var $author$project$Main$defaultContent = 'start-A\nstart-b\nA-c\nA-b\nb-d\nA-end\nb-end';
...
var $author$project$Main$parseInput = function (string) {
		...
		function (node, nodes) {
			if (nodes.$ === 'Nothing') {
				return $elm$core$Maybe$Just(
					A2($elm$core$List$cons, node, _List_Nil));
			} else {
				var xs = nodes.a;
				return $elm$core$Maybe$Just(
					A2($elm$core$List$cons, node, xs));
			}
		});
	var ruleParser = A2(
		...
		A2(
			$elm$parser$Parser$keeper,
			$elm$parser$Parser$succeed($author$project$Main$Rule),
			A2(
				$elm$parser$Parser$ignorer,
				$author$project$Utils$letters,
				$elm$parser$Parser$symbol('-'))),
		$author$project$Utils$letters);
	var parseIntoRule = function (str) {
		var _v0 = A2($elm$parser$Parser$run, ruleParser, str);
		if (_v0.$ === 'Ok') {
			var value = _v0.a;
			return $elm$core$Maybe$Just(value);
		} else {
	...
	var addRuleToCave = F2(
		function (rule, cave) {
			return A3(
				$elm$core$Dict$update,
				rule.right,
				updateCave(rule.left),
				A3(
					$elm$core$Dict$update,
					rule.left,
					updateCave(rule.right),
					cave));
		});
	return A3($elm$core$List$foldl, addRuleToCave, $elm$core$Dict$empty, rules);
};
...
var $author$project$Main$viewInput = function (input) {
	return 'number of nodes = ' + $elm$core$String$fromInt(
		...
		_List_fromArray(
			[
					...
					[
						...
						_List_fromArray(
							[
								...
								A2($author$project$Main$linkToInput, 2021, 12)),
				...
				_List_fromArray(
					[
						$elm$html$Html$text(
						'Input: ' + $author$project$Main$viewInput(model.input))
					])),
				A2(
				$elm$html$Html$div,
				_List_Nil,
