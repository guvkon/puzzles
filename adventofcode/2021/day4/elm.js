...
{
	if (region.X.J === region.ae.J)
	{
		return 'on line ' + region.X.J;
	}
	return 'on lines ' + region.X.J + ' through ' + region.ae.J;
}
...
var $author$project$Main$defaultContent = '7,4,9,5,11,17,23,2,0,14,21,24,10,16,13,6,15,25,12,22,18,20,8,19,3,26,1\n\n22 13 17 11  0\n8  2 23  4 24\n21  9 14 16  7\n6 10  3 18  5\n1 12 20 15 19\n\n3 15  0  2 22\n9 18 13 17  5\n19  8  7 25 23\n20 11 10 24  4\n14 21 16 12  6\n\n14 21 17 24  4\n10 16 15  9 19\n18  8 23 26 20\n22 11 13  6  5\n2  0 12  3  7';
	...
	function (temp, result, rawList) {
		...
		while (true) {
			...
			if (rawList.b) {
				...
				var xs = rawList.b;
				if (x === '') {
					var $temp$temp = _List_Nil,
						$temp$result = A2(append, temp, result),
						$temp$rawList = xs;
					temp = $temp$temp;
					result = $temp$result;
					rawList = $temp$rawList;
					continue groupStrings;
				} else {
					var $temp$temp = A2($elm$core$List$cons, x, temp),
						$temp$result = result,
						$temp$rawList = xs;
					temp = $temp$temp;
					result = $temp$result;
...
var $author$project$Main$parseInput = function (str) {
	var parseNumbers = function (string) {
		return A2(
			$elm$core$List$filterMap,
			$elm$core$String$toInt,
			A2($elm$core$String$split, ',', string));
	};
	var parseField = function (string) {
		...
		if (_v1.$ === 1) {
			...
			return $elm$core$Maybe$Just(
				_Utils_Tuple2(val, false));
		}
	};
	var parseRow = function (string) {
		return A2(
			$elm$core$List$filterMap,
			parseField,
	...
	if (!_v0.b) {
		return {D: _List_Nil, F: _List_Nil};
	} else {
		var x = _v0.a;
		var xs = _v0.b;
		return {
...
var $author$project$Main$init = {
	O: $author$project$Main$defaultContent,
	y: $author$project$Main$parseInput($author$project$Main$defaultContent)
};
...
var $elm$url$Url$Url = F6(
	function (protocol, host, port_, path, query, fragment) {
		return {ai: fragment, al: host, as: path, au: port_, ay: protocol, az: query};
	});
...
var $author$project$Main$update = F2(
	function (msg, model) {
		...
		return _Utils_update(
			...
			{
				O: newContent,
				y: $author$project$Main$parseInput(newContent)
			});
	});
...
var $author$project$Main$calculateWinnerBoardScore = F2(
	function (num, _v0) {
		...
		var unmarkedSum = F2(
			function (row, sum) {
				return sum + A3($elm$core$List$foldl, unmarkedRowSum, 0, row);
			});
...
var $author$project$Utils$rotateMatrix = function (matrix) {
...
var $author$project$Main$hasBoardWon = function (_v0) {
	...
	var megaBoard = A2($elm$core$List$append, board, rotatedBoard);
	var boardWon = F2(
		function (row, won) {
			return won || A3($elm$core$List$foldl, rowWon, true, row);
		});
...
var $author$project$Main$markBoards = F2(
	function (number, boards) {
		var markField = function (_v0) {
			var val = _v0.a;
			var marked = _v0.b;
			return _Utils_eq(val, number) ? _Utils_Tuple2(val, true) : _Utils_Tuple2(val, marked);
		};
		...
		var markBoard = function (board) {
			return _Utils_update(
				board,
				{
					C: A2($elm$core$List$map, markRow, board.C)
				});
		};
...
var $author$project$Main$solution1 = function (_v0) {
	var input = _v0.y;
	var maybeWinner = A2($author$project$Main$playBingo, input.F, input.D);
	if (maybeWinner.$ === 1) {
		return $elm$core$Maybe$Nothing;
	} else {
...
var $author$project$Main$solution2 = function (_v0) {
	var input = _v0.y;
	var winners = A3($author$project$Main$findAllWinners, input.F, input.D, _List_Nil);
...
var $author$project$Main$viewModel = function (model) {
	return 'numbers size = ' + _Utils_ap(
		$elm$core$String$fromInt(
			$elm$core$List$length(model.y.F)),
		', boards size = ' + $elm$core$String$fromInt(
			$elm$core$List$length(model.y.D)));
};
...
var $author$project$Main$main = $elm$browser$Browser$sandbox(
	{aY: $author$project$Main$init, a5: $author$project$Main$update, a6: $author$project$Main$view});
