...
var $author$project$Main$defaultContent = '16,1,2,0,4,2,7,1,2,14';
...
var $author$project$Main$calculateFuel1 = F2(
	function (moveTo, positions) {
		return A3(
			$elm$core$List$foldl,
			F2(
				function (pos, sum) {
					return sum + $elm$core$Basics$abs(moveTo - pos);
				}),
			0,
			positions);
	});
	...
	function (calculateFuel, input) {
		var positions = A2(
			...
			A2(
				...
				0,
				$elm$core$List$minimum(input)),
			A2(
				$elm$core$Maybe$withDefault,
				0,
				$elm$core$List$maximum(input)));
		if (!positions.b) {
			return $elm$core$Maybe$Nothing;
		} else {
			var x = positions.a;
			...
			var step = F2(
				function (pos, fuel) {
					return A2(
						$elm$core$Basics$min,
						fuel,
						A2(calculateFuel, pos, input));
				});
			return $elm$core$Maybe$Just(
				A3(
					$elm$core$List$foldl,
					step,
					A2(calculateFuel, x, input),
					xs));
		}
	});
var $author$project$Main$solution1 = function (_v0) {
	var input = _v0.input;
	return A2($author$project$Main$solve, $author$project$Main$calculateFuel1, input);
};
var $author$project$Main$calculateFuel2 = F2(
	function (moveTo, positions) {
		var len = function (pos) {
			return $elm$core$Basics$abs(moveTo - pos);
		};
		var step = F2(
			function (pos, sum) {
				var n = len(pos);
				return sum + ((((n + 1) * n) / 2) | 0);
			});
		return A3($elm$core$List$foldl, step, 0, positions);
	});
