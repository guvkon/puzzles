...
var $author$project$Main$defaultContent = 'BFFFBBFRRR\nFFFBBBFRRR\nBBFFBBFRLL';
...
var $author$project$Main$parseInput = function (str) {
	var toPass = function (bits) {
		return {
			col: A2($elm$core$List$drop, 7, bits),
			row: A2($elm$core$List$take, 7, bits)
		};
	};
	var toBit = function (chr) {
		switch (chr.valueOf()) {
			case 'F':
				return 0;
			case 'B':
...
var $author$project$Main$seatId = function (_v0) {
	var col = _v0.col;
	var row = _v0.row;
	return ($author$project$Utils$decimal(row) * 8) + $author$project$Utils$decimal(col);
};
